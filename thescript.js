...
if (window.addEventListener) {
  window.addEventListener('keydown', function (e) {
    ...
    if (e.keyCode == 50) {
      $('input[type=radio][name=action][value=3]') .prop('checked', true);
    }
    if (e.keyCode == 52) {
      $('input[type=submit][name=skip][value=skip]') .trigger('click');
    }
    if (e.keyCode == 51) {
      $('input[type=radio][name=action][value=2]') .prop('checked', true);
    }
    ...
    if (e.keyCode == 87) {
      GM_openInTab($('a[href*="https://services.okcupid.com/admin/superadmin?uid="]') .attr('href'));
    }
    if (e.keyCode == 69) {
      GM_openInTab($('a[href*="https://services.okcupid.com/admin/spammeradmin?"]') .attr('href'));
    } 
    //e
...
$('input[type=radio][name=action][value=9]') .each(function () {
  $blacklist = 0;
});
